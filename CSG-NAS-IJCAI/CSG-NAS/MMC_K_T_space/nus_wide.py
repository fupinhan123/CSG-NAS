...
def load_nus_wide(view_data_dir, n_splits=5, idx_split=0, test_size=0.2, seed=1024):
    print('********************** idx_split:', idx_split)
    view_names = ['Normalized_CH', 'Normalized_CM55', 'Normalized_CORR', 'Normalized_EDH', 'Normalized_WT','BoW_int','tags1k'] # 'BoW_int'
    x = np.load(os.path.join(view_data_dir, view_names[-1] + '.npy'))
    y = np.load(os.path.join(view_data_dir, 'y.npy'))

    train_idxs, test_idxs = split_train_test(x=x, y=y, n_splits=n_splits,test_size=test_size, seed=seed)
